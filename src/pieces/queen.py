...
from constants import WHITE_QUEEN, BLACK_QUEEN
...
class Queen(Piece):
    def __init__(self, colour, name, sameColourKing):
            ...
            self.moved = False
            self.value = 9
            self.type = WHITE_QUEEN if colour == 'w' else BLACK_QUEEN
            self.king = sameColourKing

    def getMoves(self, board):
        """
        Method which finds all the available moves that the queen can make in a given position.
